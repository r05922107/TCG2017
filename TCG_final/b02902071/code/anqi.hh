...
struct MOV {
	...
	MOV(POS s,POS e, FIN f):st(s),ed(e),st_fin(f) {
		score = 0;
	}

	bool operator==(const MOV &x) const {return st==x.st&&ed==x.ed&&st_fin==x.st_fin;}
	MOV operator=(const MOV &x) {
		st=x.st;
		ed=x.ed;
		st_fin=x.st_fin;
		return MOV(x.st, x.ed,x.st_fin);}
};
...
struct MOVLST {
	...
	void sort(){ 
		qsort(mov, num, sizeof(mov[0]), mycomp2);
	}
};
