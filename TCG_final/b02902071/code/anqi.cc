...
// UBUNTU
	...
	for(int i=0;i<8;i++) {
		// Print Index Number on the board
		for(int j=0;j<4;j++){
			fprintf(stderr,"[%02d]",mkpos(i,j));
		}

		// print Unflipped Red cnt
		if(i==2) {
			fputs("  ",stderr);
			for(int j=0;j<7;j++)
		...
		for(int j=0;j<4;j++) {
			...
			if(c == 0){  // Color is RED, fin < 7
				fprintf(stderr, ANSI_COLOR_RED " %s " ANSI_COLOR_RESET,nam[fin[mkpos(i,j)]]);
			}
			else if(c == 1){ // Color is BLACK, 7 <＝ fin < 14
				fprintf(stderr, ANSI_COLOR_CYAN " %s " ANSI_COLOR_RESET,nam[fin[mkpos(i,j)]]);	
			}
			else{
				fprintf(stderr, " %s ", nam[fin[mkpos(i,j)]]);		
			}
		...
		if(i==0) {
			fputs("  輪到 ",stderr);
			if(who==0) {
				fprintf(stderr, ANSI_COLOR_RED "紅方" ANSI_COLOR_RESET);
			} else if(who==1) {
				fprintf(stderr, ANSI_COLOR_CYAN "黑方" ANSI_COLOR_RESET);
			} else {
				fputs("？？",stderr);
			}
		} 
		...
		// print unflipped black cnt
		else if(i==2) {
			fputs("  ",stderr);
			for(int j=7;j<14;j++)
				for(int k=0;k<cnt[j];k++)
					fprintf(stderr, ANSI_COLOR_CYAN "%s" ANSI_COLOR_RESET, nam[j]);
		}
		fputc('\n',stderr);
	}
// #ifdef _WINDOWS
