...
MOV Play(BOARD &B, int remain_time) {
	MOV move = searcher.getMove(B, remain_time);
    cout<< "best move = " << move.st << "-" << move.ed << endl;
	return move;
}
...
int main(int argc, char* argv[]) {
	...
	if (argc<=1) {
	    ...
	    if(!B.ChkLose())Output(Play(B, 900000));
	    return 0;
	}
	Protocol *protocol;
